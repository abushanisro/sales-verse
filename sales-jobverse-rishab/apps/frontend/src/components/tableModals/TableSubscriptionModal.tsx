...
const TableSubscriptionModal = ({
  ...
  const [subscriptionPageNumber, setSubscriptionPageNumber] =
    useQueryState<number>("subscriptionPageNumber", 1);
  const CustomText = ({ label }: { label: string | number }) => {
    return (
      <Text ta="center" fz={{ base: 12, sm: 16 }}>
        {label}
      </Text>
    );
  };
  const queryObj = {
    pageSize: String(5),
    pageNumber: String(subscriptionPageNumber),
    subscriptionId: String(subscriptionId),
  };
  const { data, isLoading, error } =
    getQueryClient().subscription.getViewedUsersForSubscription.useQuery(
      [contract.subscription.getViewedUsersForSubscription.path, queryObj],
      {
        query: queryObj,
      }
    );
  ...
  return (
    ...
    >
              ...
              >
                <Table.Tr>
                  <Box key={index}>
                    <Table.Td c="white">
                      {" "}
                      <CustomText label={jobs.name} />
                    </Table.Td>
                  </Box>
                  <Table.Td c="white">
                    <CustomText label={getIndianDate(jobs.viewDate)} />
                  </Table.Td>
                  <Table.Td c="white">
                    <CustomText label={getIndianDate(jobs.expiryDate)} />
                  </Table.Td>
                </Table.Tr>
              </Table.Tbody>
            ))}
      ...
      <Center mt={20}>
        <CustomPagination
          size={isMobile ? "sm" : "md"}
          value={subscriptionPageNumber}
          onChange={(e: number) => {
            setSubscriptionPageNumber(e);
          }}
...
export default TableSubscriptionModal;
