...
import { MyPaymentHeading } from "@/types/employer";
...
import { PostPaidHistoryResponseType } from "@/types/postPaid";
...
const MyPaymentList = () => {
  ...
  const { data, isLoading, error } =
    getQueryClient().paidJobs.getSubscriptionHistory.useQuery(
      [contract.paidJobs.getSubscriptionHistory.path, queryObj],
      {
        query: queryObj,
      },
      {
  ...
  return (
    <DetailedPaginatedTable
      ...
      tableBody={(data: PostPaidHistoryResponseType[]) => {
        if (data.length === 0) {
          return (
        ...
        return data.map((payment: PostPaidHistoryResponseType) => {
          return (
